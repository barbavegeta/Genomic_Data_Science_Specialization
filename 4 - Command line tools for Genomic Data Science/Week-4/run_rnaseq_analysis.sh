...
stringtie -e -B -G merged.gtf -o Cuffdiff/Day8.gtf Tophat/Day8/accepted_hits.bam
stringtie -e -B -G merged.gtf -o Cuffdiff/Day16.gtf Tophat/Day16/accepted_hits.bam
...
# --- BEGIN FIX FOR prepDE.py INPUT ---
# You need to explicitly list sample IDs and their corresponding GTF files.
# The 'ls -1' command will NOT produce the required format.
echo -e "Day8\tCuffdiff/Day8.gtf" > cuffdiff_gtfs.txt
echo -e "Day16\tCuffdiff/Day16.gtf" >> cuffdiff_gtfs.txt
# --- END FIX FOR prepDE.py INPUT ---

# Check if prepDE.py exists before running
if command -v prepDE.py &> /dev/null
then
    prepDE.py -i cuffdiff_gtfs.txt
    ...
    echo "WARNING: prepDE.py script not found. Skipping count matrix generation."
    echo "Please download it from the StringTie website and ensure it is in your PATH."
fi
